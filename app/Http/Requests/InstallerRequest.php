...
class InstallerRequest extends FormRequest
{
    ...
    public function rules()
    {
        $rules = [
            ...
            'email' => [
                'email',
                'nullable',
                Rule::unique('installers')->where('company_id', $this->header('company')),
            ],
            'password' => [
                'nullable',
            ],
        ...
        if ($this->isMethod('PUT') && $this->email != null) {
            $rules['email'] = [
                'email',
                'nullable',
                Rule::unique('installers')->where('company_id', $this->header('company'))->ignore($this->route('installer')->id),
            ];
        }
    ...
    public function getInstallerPayload()
    {
