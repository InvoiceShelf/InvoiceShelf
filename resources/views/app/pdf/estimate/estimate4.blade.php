    ...
    <table width="100%">
        <tr>
            ...
            <td class="header-right company-details">
                ...
                <p class="company-add">Αριθμός εγγραφής: ΗΕ 379350</p>
                <p class="company-add">ΑΦΤ: 10379350Ζ</p>
                <p class="company-add">Σάμου 5, Τρεμιθούσα 8270</p>
                <p class="company-add">CY-8270, Πάφος, Κύπρος</p>
...
<div class="header-table table-border">
    <table width="100%" class="text-center" style="padding-left:30px; padding-right:30px;">
        <thead>
        <tr>
            <th bgcolor="#9BC2E6" style="font-size:12px;">Είδος Παραστατικού</th>
            <th bgcolor="#9BC2E6" style="font-size:12px;">Σειρά</th>
            <th bgcolor="#9BC2E6" style="font-size:12px;">Αριθμός</th>
            <th bgcolor="#9BC2E6" style="font-size:12px;">Ημ/νία Έκδοσης</th>
        </tr>
        </thead>
        <tbody>
        <tr>
            <td class="text-center" style="font-size:12px;">ΠΡΟτιμολόγιο παροχής υπηρεσιών</td>
            <td class="text-center" style="font-size:12px;"></td>
            <td class="text-center" style="font-size:12px;">{!! str_replace('AP-','',$estimate->invoice_number) !!}</td>
            <td class="text-center" style="font-size:12px;">{{$estimate->formattedInvoiceDate}}</td>
        </tr>
        </tbody>
    </table>
</div>
<div class="header-table table-border">
    <table width="100%" style="padding-left:30px; padding-right:30px;">
        <thead>
        <tr>
            <th colspan="2" bgcolor="#9BC2E6">Στοιχεία πελάτη</th>
        </tr>
        </thead>
        <tbody>
        <tr>
            <th>Επωνυμία</th>
            <td>{{$estimate->customer->billingAddress->name}}</td>
        </tr>
        <tr>
            <th>Διεύθυνση</th>
            <td>
                {!! nl2br(htmlspecialchars($estimate->customer->billingAddress->address_street_1)) !!}
                {!! nl2br(htmlspecialchars($estimate->customer->billingAddress->address_street_2)) !!}
                {{$estimate->customer->billingAddress->zip}}
            </td>
        </tr>
        @if($estimate->customer->billingAddress->country && $estimate->customer->billingAddress->country->name)
            <tr>
                <th>Χώρα</th>
                <td>{{$estimate->customer->billingAddress->country->name}}</td>
            </tr>
        @endif
        <tr>
            <th>Νομός</th>
            <td>{!! nl2br(htmlspecialchars($estimate->customer->billingAddress->state)) !!}</td>
        </tr>
        <tr>
            <th>Α.Φ.Μ.</th>
            <td>{!! nl2br(htmlspecialchars($estimate->customer->fields[0]->string_answer)) !!}</td>
        </tr>
        @if($estimate->customer->billingAddress->phone)
            <tr>
                <th>Τηλέφωνο</th>
                <td>{{$estimate->customer->billingAddress->phone}}</td>
            </tr>
        @endif
        </tbody>
    </table>
...
<div class="wrapper">
    <table width="100%" class="table2 table-border" cellspacing="0" border="0">
        <tr class="main-table-header" bgcolor="#9BC2E6">
            ...
            <th width="50%" class="ItemTableHeader" style="text-align: left; color: #55547A; padding-left: 4px">Υπηρεσία</th>
            <th class="ItemTableHeader" style="text-align: right; color: #55547A; padding-right: 20px">Ποσ.</th>
            <th class="ItemTableHeader" style="text-align: right; color: #55547A; padding-right: 20px">Τιμή</th>
            @if($estimate->discount_per_item === 'YES')
                <th class="ItemTableHeader" style="text-align: right; color: #55547A; padding-left: 10px">Έκπτωση</th>
            @endif
            <th class="ItemTableHeader" style="text-align: right; color: #55547A;">Σύνολο</th>
        </tr>
        @php
            $index = 1
        @endphp
...
<div class="total-display-container">
    <table width="100%" cellspacing="0px" style="margin-left:400px; margin-top: 10px" border="0" class="total-display-table @if(count($estimate->items) > 12) page-break @endif">
        <tr>
            <td class="no-border" style="color: #55547A; padding-left:10px;  font-size:12px;">Συνολική αξία</td>
            <td class="no-border items padd2" style="padding-right:10px; text-align: right;  font-size:12px; color: #040405;">{!! format_money_pdf($estimate->sub_total, $estimate->customer->currency) !!}</td>
        </tr>
        ...
        <tr>
            <td class="no-border total-border-left" style="padding-left:10px; padding-bottom:10px; text-align:left; padding-top:10px; font-size:12px;  color: #55547A;">
                <label class="total-bottom">Ποσό πληρωμής</label>
            </td>
...
@if ($estimate->paid_status != 'PAID')
    ...
    <div class="header-table table-border margin:0 auto;">
        <table width="600" cellspacing="0px" style="margin-left:100px; margin-top: 10px" border="0" class="table3">
            <tbody>
            <tr>
                <th  bgcolor='9BC2E6' colspan='2'>Λογαριασμοί κατάθεσης χρημάτων</th>
            </tr>
            <tr>
                <th>IBAN</th>
            ...
            <tr>
                <th>Αιτιολογία</th>
                <td>{!! str_replace('-','',$estimate->invoice_number) !!}</td>
            </tr>
            <tr>
                <th colspan="2" bgcolor="#9BC2E6">Εναλλακτικοί τρόποι πληρωμής</th>
            </tr>
            <tr>
                <th>PayPal</th>
            ...
            <tr>
                <th>Εξόφληση Online</th>
                <td><a href="https://paypal.me/eSolutionsSofthouse/{{ number_format($estimate->total / 100,2) }}" target="_blank">Κάντε κλικ εδώ</a></td>
            </tr>
            </tbody>
        </table>
    </div>
