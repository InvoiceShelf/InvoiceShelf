<template>
  ...
  >
            ...
            >
              <BaseInputGroup class="pt-2 -mt-4">
                <BaseRadio
                  ...
                  :label="$t('installers.create_date')"
                  size="sm"
                  name="filter"
    ...
    <div
      ...
      "
    >
      <div v-for="(installer, index) in installerList" :key="index">
        <router-link
          v-if="installer"
          :id="'installer-' + installer.id"
          :to="`/admin/installers/${installer.id}/view`"
          :class="[
            'flex justify-between p-4 items-center cursor-pointer hover:bg-gray-100 border-l-4 border-transparent',
            {
              'bg-gray-100 border-l-4 border-primary-500 border-solid':
                hasActiveUrl(installer.id),
            },
          ]"
      ...
      <p
        v-if="!installerList?.length && !isFetching"
        class="flex justify-center px-4 mt-5 text-sm text-gray-600"
      >
        {{ $t('installers.no_matching_installers') }}
      </p>
    </div>
  </div>
</template>
...
const installerList = ref(null)
const currentPageNumber = ref(1)
const lastPageNumber = ref(1)
const installerListSection = ref(null)
...
async function loadInstallers(pageNumber, fromScrollListener = false) {
  if (isFetching.value) {
    return
  }
  ...
  let response = await installerStore.fetchInstallers({
    page: pageNumber,
    ...params,
    limit: 15
  })
  ...
  installerList.value = installerList.value ? installerList.value : []
  installerList.value = [...installerList.value, ...response.data.data]

  currentPageNumber.value = pageNumber ? pageNumber : 1
  lastPageNumber.value = response.data.meta.last_page
  let installerFound = installerList.value.find(
  ...
  if (
    ...
    loadInstallers(++currentPageNumber.value)
  }
...
function scrollToInstaller() {
  const el = document.getElementById(`installer-${route.params.id}`)
...
function addScrollListener() {
  installerListSection.value.addEventListener('scroll', (ev) => {
    if (
      ...
      if (currentPageNumber.value < lastPageNumber.value) {
        loadInstallers(++currentPageNumber.value, true)
      }
    }
  })
}
